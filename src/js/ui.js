...
const UI = {
            ...
            <div class="track-card flex items-center gap-4 p-3 rounded-lg cursor-pointer ${isPlaying ? 'playing' : ''}" data-track-index="${index}" data-track-id="${track.id}">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <button class="download-track-btn p-2 text-gray-400 hover:text-white transition-colors" data-track-id="${track.id}" title="Download MP3" ${typeof Downloader !== 'undefined' && !Downloader.isLocal ? 'hidden' : ''}>
                            <svg class="w-4 h-4 download-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                            </svg>
                            <svg class="w-4 h-4 download-spinner hidden animate-spin" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                            </svg>
                        </button>
    ...
    attachTrackListeners(container) {
        ...
        // Download buttons (local installs only -- needs yt-dlp + ffmpeg)
