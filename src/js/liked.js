...
const LikedSongs = {
            ...
            return `
                <div class="track-card flex items-center gap-4 p-3 rounded-lg cursor-pointer hover:bg-dark-100 transition-colors ${isPlaying ? 'playing' : ''}" data-track-index="${index}">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        ...
                        <button class="download-liked-btn p-2 text-gray-400 hover:text-green-400 transition-colors" data-liked-index="${index}" title="Download">
    ...
    _attachLikedViewListeners(container) {
        ...
        // Download buttons
        container.querySelectorAll('.download-liked-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const index = parseInt(btn.dataset.likedIndex);
                if (this.songs[index]) {
                    Downloader.download(this.songs[index]);
                }
            });
        });
    },
